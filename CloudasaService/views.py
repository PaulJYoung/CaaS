...
def comment(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            p = post.objects.create(firstname=firstname, surname=surname, emailaddr=emailaddr, comment=comment)
    else:
        form = Comment()
    return render(request, 'CloudasaService/comment.html', {'form': form})
